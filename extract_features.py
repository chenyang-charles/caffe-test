...
def theoreticRF(info, size, stride, filepath):
	"""
		Get the theretic RF region of given image
	"""
	global H
	global W
	...
	x = info[1]
	y = info[2]
	img = resize(imread(filepath), H, W)
	RF = img[x * stride : x * stride + size, y * stride : y * stride + size]
